        ...
        try :
            ...
            # 주소 검색 불가
                ...
                try :
                    ...
                    # 불필요한 주소 제거
                    for j in range(len(0, address_list)) :
                        if address_list[j][-1] not in ["시", "도", "군", "구", "읍", "면", "동"] :
                            # 숫자 주소인지 판별
                            try :
                                 temp = int(address_list[j][0])
                            except :
                                if j != 0 :
                                    address_list.pop(j)
                                falsestack += 1
                                #print("new" + str(address_list))
                    # for 문에서 삭제된 주소가 없다면
                ...
                if int(data['result']['resultdata'][0]['addr_type']) >= 5 :
                    print(data)
