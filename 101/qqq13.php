...
$db_selected=@mysqli_connect(DB_HOST,DB_USER,DB_PASSWORD,DB_NAME) OR die('Could not to connect to Mysql:'.mysqli_connect_error());
  ...
  $sql1 = "SELECT test.city,weather.dangdiqiya FROM test,weather where test.KID=weather.KID and test.city='$name' and weather.date >='$start_time' AND weather.date < '$end_time' group by time";
