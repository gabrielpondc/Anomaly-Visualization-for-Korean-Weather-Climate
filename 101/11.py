...
df = pd.read_sql('SELECT test.city,weather.* FROM weather,test where weather.KID=test.KID and test.city=\'부산광역시\' and weather.date >=\'2019-12-01\' AND weather.date <\'2019-12-30\'', con=db_connection)
df = df.iloc[:,3:]
...
P=list(np.abs(lof.negative_outlier_factor_))
print(len(P))